...
class Window(QMainWindow):
	...
	def check_position(self) -> bool:
		...
		if (self.ui.groupBox.x() + (self.brushSize + 1) // 2) < window_pos.x() < \
			(self.ui.groupBox.x() + self.ui.groupBox.width() - (self.brushSize + 1) // 2) and \
			(self.ui.groupBox.y() + (self.brushSize + 1) // 2) < window_pos.y() < \
			(self.ui.groupBox.y() + self.ui.groupBox.height() - (self.brushSize + 1) // 2):
			return True
		else:
			return False
	...
	def recognize_picture(self):
		...
		self.setProgressBars(self.neural_network.recognize_picture(image))

	def clearDisplay(self):
		color = QColor(240, 240, 240)
		self.image.fill(color)
		self.update()
		self.setProgressBars([[0]*10])
